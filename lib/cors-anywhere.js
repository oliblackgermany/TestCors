...
let net = require('net');
let url = require('url');
let regexp_tld = require('./regexp-top-level-domain');
let getProxyForUrl = require('proxy-from-env').getProxyForUrl;
...
function onProxyResponse(proxy, proxyReq, proxyRes, req, res) {
  let requestState = req.corsAnywhereRequestState;
...
function getHandler(options, proxy) {
  ...
  let hasRequiredHeaders = function(headers) {
  ...
  return function(req, res) {
    ...
    let isRequestedOverHttps = req.connection.encrypted || /^\s*https/.test(req.headers['x-forwarded-proto']);
    let proxyBaseUrl = (isRequestedOverHttps ? 'https://' : 'http://') + req.headers.host;
